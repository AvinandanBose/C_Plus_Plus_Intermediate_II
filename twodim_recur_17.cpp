...
void showUpperTriangularMatrix(int[][10], int, int, int, int);
using namespace std;
int main()
{
...
void showUpperTriangularMatrix(int arr[][10], int rows, int columns, int i, int j)
{
    if (i == rows)
    {
        return;
    ...
    {
        cout << endl;
         showUpperTriangularMatrix(arr, rows, columns, i + 1, 0);
         return;
    }
    if (i < j)
    {
     ...
     showUpperTriangularMatrix(arr, rows, columns, i, j + 1);
}