...
int showLowerTriangularMatrix(int[][10], int, int, int, int);
using namespace std;
int main()
{
...
int showLowerTriangularMatrix(int arr[][10], int rows, int columns, int i, int j)
{
    if (i == rows)
    {
        return 0;
    ...
    {
        cout << endl;
        return showLowerTriangularMatrix(arr, rows, columns, i + 1, 0);
    }
    if (j<i)
    {
        cout << arr[i][j] << " ";
    }
    return showLowerTriangularMatrix(arr, rows, columns, i, j + 1);
}