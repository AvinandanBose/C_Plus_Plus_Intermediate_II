...
int add(int [][10], int [][10], int [][10], int , int , int, int ) ;
using namespace std;
int main()
{
    ...
    if (rowsize == colsize)
    {
        ...
        add(mat1, mat2, result, rowsize, colsize, 0, 0);
...
int add(int mat1[][10],int mat2[][10],int result[][10],  int rows, int columns, int i, int j)
{
    if(i==rows){
    ...
    if(i< rows){
        if(j< columns){
            ...
            add(mat1, mat2, result, rows, columns, i, j+1);
        }
        else{
            add(mat1, mat2, result, rows, columns, i+1, 0);
        }
    }
