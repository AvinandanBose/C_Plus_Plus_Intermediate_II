// Diagonal  of A Square Matrix using Recursion

#include <iostream>
int input(int[][10], int, int);
void display(int, int, int[][10]);
void showDiagonalOfMatrix(int[][10], int, int, int, int);
using namespace std;
int main()
{
...
void showDiagonalOfMatrix(int arr[][10], int rows, int columns, int i, int j)
{
    ...
    {
        cout << endl;
         showDiagonalOfMatrix(arr, rows, columns, i + 1, 0);
         return;
    }
    if (i == j)
    {
    ...
    else
    {
     ...
     showDiagonalOfMatrix(arr, rows, columns, i, j + 1);
}